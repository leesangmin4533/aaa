...
- **통합 데이터베이스 관리**: 모든 수집 데이터는 날짜별 파일이 아닌, `code_outputs/integrated_sales.db`라는 단일 통합 데이터베이스에 누적 저장됩니다.
- **자동 과거 데이터 수집**: `main.py` 실행 시, 통합 DB를 먼저 확인하여 최근 7일 중 누락된 날짜의 데이터가 있으면 해당 날짜의 데이터를 자동으로 수집합니다.
