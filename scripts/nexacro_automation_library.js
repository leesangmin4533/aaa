(() => {
  ...
  async function getNestedNexacroComponent(pathComponents, initialScope, timeout = 30000) {
    ...
    for (let i = 0; i < pathComponents.length; i++) {
      ...
      if (currentScope[componentId]) {
        currentScope = currentScope[componentId];
        console.log(`[getNestedNexacroComponent] 속성 직접 접근: ${pathComponents.slice(0, i + 1).join('.')}`);
      } else if (componentId === 'form') {
        // 'form'은 속성으로 직접 접근 (보조 로직)
        currentScope = currentScope.form;
        if (!currentScope) {
          throw new Error(`'${pathComponents.slice(0, i).join('.')}' 내에 'form' 속성을 찾을 수 없습니다.`);
        }
        console.log(`[getNestedNexacroComponent] 속성 접근 (form): ${pathComponents.slice(0, i + 1).join('.')}`);
      } else {
